...
void GrowtopiaBot::onLoginRequested() {
	...
	loginVar.append("f|1");
	loginVar.append("protocol|161"); //sus
	...
	loginVar.append("meta|TLddQ2jYAo");
	...
	loginVar.append("UUIDToken|01B4B4E14BAFC0F90B856C1189BC6407");
	loginVar.set("UUIDToken", "01B4B4E14BAFC0F90B856C1189BC6407");
	loginVar.set("mac", mac);
	loginVar.set("platformID", "4");
	loginVar.remove("fz");

	if (user && token && UUIDToken) {
		loginVar.append("user|" + std::to_string(user));
		loginVar.append("token|" + std::to_string(token));
		loginVar.append("UUIDToken|" + std::to_string(UUIDToken));
...
void GrowtopiaBot::onPacketCallFunction(gameupdatepacket_t* packet) {
	...
	if (!function.empty()) {
		if (function == "OnSendToServer") {
		    ...
		    UUIDToken = varlist.get(5).get_uint32();
